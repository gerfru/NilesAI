...
class TestFetchWaHistory:
    ...
    async def test_empty_messages(self, wa_action):
        messages, has_more = await _fetch_wa_history(wa_action, "435000000000", "niles-wa-1")
        assert messages == []
        assert has_more is False
        wa_action.fetch_messages.assert_awaited_once_with("435000000000@s.whatsapp.net", instance="niles-wa-1")
    ...
    async def test_trigger_message_is_user_role(self, wa_action):
        ...
        messages, _ = await _fetch_wa_history(wa_action, "435000000000", "inst")
    ...
    async def test_mixed_conversation(self, wa_action):
        ...
        messages, _ = await _fetch_wa_history(wa_action, "435000000000", "inst", limit=20)
    ...
    async def test_pagination_first_page(self, wa_action):
        ...
        messages, has_more = await _fetch_wa_history(wa_action, "435000000000", "inst", limit=2, offset=0)
    ...
    async def test_pagination_second_page(self, wa_action):
        wa_action.fetch_messages.return_value = [_make_raw_msg(f"msg-{i}", ts=1000 + i) for i in range(5)]
        messages, has_more = await _fetch_wa_history(wa_action, "435000000000", "inst", limit=2, offset=2)
    ...
    async def test_pagination_last_page(self, wa_action):
        wa_action.fetch_messages.return_value = [_make_raw_msg(f"msg-{i}", ts=1000 + i) for i in range(5)]
        messages, has_more = await _fetch_wa_history(wa_action, "435000000000", "inst", limit=2, offset=4)
    ...
    async def test_pagination_beyond_end(self, wa_action):
        ...
        messages, has_more = await _fetch_wa_history(wa_action, "435000000000", "inst", limit=20, offset=5)
    ...
    async def test_jid_construction(self, wa_action):
        ...
        await _fetch_wa_history(wa_action, "435000000000", "niles-wa-1")
