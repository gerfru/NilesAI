...
class TestGetStatus:
    async def test_known_phone_returns_connected(self):
        settings = _make_settings(signal_phone_number="+4366012345678")
        action = _make_action()

        ctx = await action.get_status(settings)

        assert ctx["signal_status"] == "connected"
        assert ctx["signal_phone"] == "+4366012345678"
    ...
    async def test_auto_discovers_phone(self):
        settings = _make_settings()
        sa = AsyncMock()
        sa.get_accounts.return_value = ["+4366012345678"]
        store = AsyncMock()
        ...
        assert ctx["signal_status"] == "connected"
        assert ctx["signal_phone"] == "+4366012345678"
        assert ctx["phone_discovered"] == "+4366012345678"
        store.set.assert_awaited_once_with("signal_phone_number", "+4366012345678")
...
class TestDisconnect:
    async def test_unlinks_and_clears_db(self):
        ...
        await action.disconnect("+4366012345678")

        sa.unlink.assert_awaited_once_with("+4366012345678")
