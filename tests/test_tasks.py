...
class TestListTasks:
    async def test_empty_list(self, action):
        action._client.get = AsyncMock(return_value=_resp([]))
        result = await action.list_tasks()
        assert result == []

    async def test_returns_simplified(self, action):
        action._client.get = AsyncMock(return_value=_resp(SAMPLE_TASKS))
        result = await action.list_tasks()
    ...
    async def test_filter_done(self, action):
        ...
        call_kwargs = action._client.get.call_args
    ...
    async def test_include_done(self, action):
        ...
        action._client.get = AsyncMock(return_value=_resp(SAMPLE_TASKS_WITH_DONE))
...
class TestCreateTask:
    async def test_minimal(self, action):
        ...
        action._client.get = AsyncMock(return_value=_resp(SAMPLE_PROJECTS))
        action._client.put = AsyncMock(return_value=_resp(SAMPLE_CREATED_TASK))
        ...
        put_call = action._client.put.call_args
...
class TestCompleteTask:
    async def test_found_and_completed(self, action):
        ...
        action._client.get = AsyncMock(return_value=_resp(SAMPLE_TASKS))
        action._client.post = AsyncMock(return_value=_resp({"done": True}))
        ...
        post_call = action._client.post.call_args
    ...
    async def test_ambiguous(self, action):
        ...
        action._client.get = AsyncMock(return_value=_resp(tasks_with_overlap))
...
class TestDefaultProject:
    async def test_caches_default_project(self, action):
        ...
        assert action._client.get.call_count == 1
