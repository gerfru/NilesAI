...
Standardmaessig wird `https://dav.mailbox.org/carddav/` verwendet. Fuer andere Anbieter die `CARDDAV_URL` in `.env` anpassen:
...
3. Die Tailscale-IP oder den MagicDNS-Namen notieren (z.B. `niles.tail1d4a0f.ts.net`)
...
| `CARDDAV_URL` | nein | `https://dav.mailbox.org/carddav/` | CardDAV-Server URL |
| `CARDDAV_USER` | nein | -- | CardDAV-Benutzername |
| `CARDDAV_PASSWORD` | nein | -- | CardDAV-Passwort |
| `CALDAV_URL` | nein | `https://dav.mailbox.org/caldav/` | CalDAV-Server URL (Legacy) |
