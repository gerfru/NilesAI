...
```
https://whatsapp.home.lab/manager
```
...
No manual account creation or token generation required. The Vikunja web UI (`https://vikunja.home.lab`) is available for direct task management.
...
| Subdomain | Service |
| --------- | ------- |
| niles.home.lab | Niles Core (Web UI + API) |
| whatsapp.home.lab | Evolution API Manager |
| vikunja.home.lab | Vikunja Web UI |
...
3. Configure Tailscale Split-DNS to route `*.home.lab` to the Mac Mini's Tailscale IP
4. Set `BASE_URL` in `.env`:

```bash
BASE_URL=https://niles.home.lab
```

5. Restart: `./scripts/start.sh`

Niles is now accessible from any device on the Tailscale network via `https://niles.home.lab`.
...
| `VIKUNJA_PUBLIC_URL` | External URL for nav link + web UI (`https://vikunja.home.lab`) |
...
External HTTPS access is provided by homelab-gateway via subdomains (niles.home.lab, whatsapp.home.lab, vikunja.home.lab).
